...
OPEN fastforward_product_cursor
	...
	BEGIN--cursor cicle
		SET @Counter1 = 1
		SELECT @rndNum = ROUND((RAND()*6),0)--num of products to be added
...
OPEN fastforward_product_cursor
	...
	begin
		...
		IF (@rndNum2=0)/* if 0 has no user id */
			...
			VALUES(NULL,@ProductId,'Name','LastName','email'+CAST(NEWID() as varchar(36))+'@gmail.com'
		...
		ELSE
			...
			SELECT @email=Email FROM Account JOIN [User] ON Account.Id=[User].AccountId where [User].Id=@userId
